...
const selectDefaultGrade = (content: Content) => content.grades?.[0] ?? 2;

const buildSkillState = (skills: Content['skills']): Record<string, SkillState> => {
  return skills.reduce<Record<string, SkillState>>((acc, skill) => {
    acc[skill.id] = {
      difficulty: 1,
      correctStreak: 0,
      incorrectStreak: 0,
...
const sanitizeGrade = (grade: number, content: Content) => {
...
const sanitizeSkillState = (
  skillState: Record<string, SkillState>,
  content: Content
): Record<string, SkillState> => {
  const stateCopy: Record<string, SkillState> = {};
...
export const createInitialSave = (
  content: Content,
  {
    ...
    grade,
    selectedCreatureId,
  }: { parentEmail?: string; grade?: number; selectedCreatureId?: string }
): SaveData => {
  ...
  const validatedGrade = sanitizeGrade(
    grade ?? selectDefaultGrade(content),
    content
  );
...
const normalizeSave = (raw: unknown, content: Content): SaveData | null => {
...
export const persistSave = (save: SaveData) => {
...
const scoreFromLegacyLevel = (level: number | null) => {
...
export const migrateLegacyData = (content: Content): SaveData | null => {
  const storage = getStorage();
  if (!storage) {
    return null;
  }
...
export const loadSave = (content: Content): SaveData | null => {
  const storage = getStorage();
  if (!storage) {
    return null;
  }
...
export const updateSave = (save: SaveData, content: Content): SaveData => {
